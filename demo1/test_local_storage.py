...
def test_get_token():
    url = "http://amp.test.alwooo.com:8497/api/common/login"
...
class Test_compare_data():
    ...
    def test_login(self):
        #使用JS注入localstorage跳过登录
        sleep(3)
        self.dr.execute_script('window.$system.saveToken(%s);' % test_get_token())
        sleep(1)

        self.dr.refresh()
        #需要重新跳转页面
        self.search.open("http://amp.test.alwooo.com:8497/home")
